...
def do_lines_intersect_region(S, E, L1, L2, tolerance=1e-3):
    ...
    if np.isclose(X1, X2, atol=tolerance) and (sx - Xave)*(ex - Xave) <= 0 and max(min(sy, ey), min(Y1, Y2)) <= min(max(sy, ey), max(Y1, Y2)):
        intersectY = (sy + ey) * (sx - Xave) / (sx - ex)
        return min(Y1, Y2) <= intersectY <= max(Y1, Y2)
    
    elif np.isclose(Y1, Y2, atol=tolerance) and (sy - Yave)*(ey - Yave) <= 0 and max(min(sx, ex), min(X1, X2)) <= min(max(sx, ex), max(X1, X2)):
        intersectX = (sx + ex) * (sy - Yave) / (sy - ey)
        return min(X1, X2) <= intersectX <= max(X1, X2)
    
    return False

print(do_lines_intersect_region(
    (4,2,0),
    (9,-2,0),
    (0,0),
    (5,0)
...
def a_star_3d(start, goal, boundaries):
    ...
    while open_set:
        ...
        neighbors = [(current[0] + dx, current[1] + dy, current[2] + dz) for dx, dy, dz in [(-0.01, 0, 0), (0.01, 0, 0), (0, -0.01, 0), (0, 0.01, 0), (0, 0, -0.2), (0, 0, 0.2)]]
        for neighbor in neighbors:
            if neighbor in f_score or not is_passable(current, neighbor, boundaries):
                continue
...
def calculate_manhattan_distance_matrix_with_boundaries(df, boundaries):
    ...
    for i in range(num_shelves):
        for j in range(num_shelves):
            if i < j:
                path = a_star_3d((coords[i][0], coords[i][1], coords[i][2]), (coords[j][0], coords[j][1], coords[j][2]), boundaries)
                if path:
