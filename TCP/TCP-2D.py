...
df = pd.read_excel(r'Stamm_Lagerorte-1.xlsx') # Import Dataframe
...
def score_based_color(shelf, start_shelf, boundaries):
    start = (df[df['Regal/Fach/Boden'] == (start_shelf)][['X', 'Y', 'Z']].values)[0]
    current = (df[df['Regal/Fach/Boden'] == (shelf)][['X', 'Y', 'Z']].values)[0]
    ...
    score = scoreDist * 0.5 + scoreAisle * 0.3 + scoreGround * 0.2
    
    return f"#{int(255*score/500):02x}{int(255*(500-score)/500):02x}00"
...
def visualize_path_3d(df, boundaries, path, start_shelf):
    ...
    for index, shelf in df.iterrows():
        fig.add_trace(go.Scatter3d(
            ...
            z=[shelf['Z']],
            mode="markers+text",
            marker=dict(size=5, color=score_based_color(shelf['Regal/Fach/Boden'], start_shelf, boundaries)),
